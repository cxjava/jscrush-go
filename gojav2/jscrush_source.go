...
var jscrusher = (function() {
    ...
    function generateOutput(swaps, leftover) {
        ...
        output += ';for(Y in $="' + swaps.map(function(swap) {
            return swap.to;
        }).join('');
    ...
    function crush(inputscript, strategy) {
        ...
        while (!stop) {
            safechars = calcsafechars(leftover);
            // we can gain some extra rounds
        ...
        var output = '_=' + smartQuotes(leftover);
        output += ';for(Y in $="' + swaps.reverse().map(function(swap) {
            return swap.to;
        }).join('');
        output += '")with(_.split($[Y]))_=join(pop());eval(_)';
