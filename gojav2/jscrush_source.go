...
// http://jquerymy.com/jscrush/
var jscrush_src = `"use strict";

var jscrush = function(s0) {
			// JavaScript Crush by Aivo Paas
			// with some corrections by ermouth			
			var Q=[],m,B,o,s,N,e,c,i=s=s0,X=B=s.length/2,O=m='',x,j,t,R;
			for (i=1000;--i;(i>31)&&i-34&&i-39&&i-92&&i-127&&Q.push(String.fromCharCode(i)));
			for(var S=encodeURI(i).replace(/%../g,'i').length;;m=c+m){
				for(var M=N=e=c=0,i=Q.length;!c&&--i;!~s.indexOf(Q[i])&&(c=Q[i]));
				if(!c)break;
				if(O){
					o={};
